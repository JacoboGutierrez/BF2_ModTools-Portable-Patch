...
REM Make the build independent of the caller's current directory and drive.
set "MUNGE_BUILD_DIR=%~dp0"
for %%I in ("%MUNGE_BUILD_DIR%..\..") do set "MODTOOLS_ROOT=%%~fI"
if not exist "%MODTOOLS_ROOT%\ToolsFL\Bin\ConfigMunge.exe" (
	echo ERROR: Unable to locate ToolsFL\Bin relative to "%MUNGE_BUILD_DIR%".
	exit /b 1
)
pushd "%MUNGE_BUILD_DIR%" || exit /b 1
set "MUNGE_BIN_DIR=%MODTOOLS_ROOT%\ToolsFL\Bin"
set "PATH=%MUNGE_BIN_DIR%;%PATH%"
