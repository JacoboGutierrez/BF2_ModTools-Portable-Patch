...
Ejecuta `BF2_ModTools_Portable.cmd` sin parámetros. El lanzador abre:

```text
data\_BUILD\Modtools VisualMunge.exe
```

Esta instancia de VisualMunge crea un directorio, por ejemplo, `data_ABC`.
...
El parche también permite que los nombres de las carpetas de proyecto contengan
espacios y paréntesis. Por ejemplo:
...
VisualMunge continúa usando un código interno de exactamente tres letras. Crea
primero el proyecto como `LEG` o `ZMB` y después renombra la carpeta externa.
Las carpetas y referencias internas del mundo deben conservar el código `LEG` o
`ZMB` original.
...
Los scripts de compilación corregidos localizan `ToolsFL\Bin` respecto de la
carpeta raíz de `BF2_ModTools`. Por eso funcionan desde otra unidad y en
carpetas cuyos nombres contienen espacios.

El ZIP modifica `data` y `assets\bkupmungebats`, de modo que los proyectos
creados después de instalar el parche reciben los scripts portátiles.
